...
export type PayrollRecord = {
  ...
  name: string;
  position: string;
  employeeId?: string;  // Added for HR/Manager view
  baseSalary: number;
  bonuses: number;
...
// Props for the PayrollComponent
interface PayrollComponentProps {
  userRole: 'employee' | 'manager' | 'hr';
  apiEndpoint: string;  // Different API endpoints for different roles
  title?: string;       // Custom title
  showEmployeeName?: boolean;  // Show employee name column (true for managers/HR)
}
...
function PayrollComponent({
  userRole,
  apiEndpoint,
  title = userRole === 'employee' ? 'My Payroll' : 'Payroll Management',
  showEmployeeName = userRole !== 'employee'
  ...
  const handleDownloadPayslip = async (record: PayrollRecord) => {
    ...
    try {
      // Use different endpoints based on role if needed
      let payslipEndpoint = '/api/employee/payroll/payslip';
      if (userRole === 'hr') {
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow dark:shadow-gray-700">
          <div className="p-6 border-b dark:border-gray-700">
            <h2 className="text-xl font-semibold dark:text-white">
              {userRole === 'employee' ? 'Salary History' : 'Payroll Records'}
            </h2>
          </div>
                ...
                <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                  {payrollRecords.map((record) => (
                    <tr key={record._id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap dark:text-gray-300">PKR {record.baseSalary.toLocaleString()}</td>
                      <td className="px-6 py-4 whitespace-normal dark:text-gray-300">
                        <div className="flex flex-col">
                          <span className="text-green-600 dark:text-green-400">
                            +PKR {record.bonuses.toLocaleString()}
                          </span>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap font-semibold dark:text-white">PKR {record.netSalary.toLocaleString()}</td>
